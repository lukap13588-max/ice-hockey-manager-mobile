...
A.init=function(){A.enabled=I.state?.audio?.enabled!==false;A.menu=new Audio("./assets/audio/menu-loop.wav");A.menu.loop=true;A.menu.volume=.24;A.crowd=new Audio("./assets/audio/crowd-loop.wav");A.crowd.loop=true;A.crowd.volume=.16};
...
A.sfx=function(name,volume=.8){if(!A.unlocked||!A.enabled)return;const a=new Audio(`./assets/audio/${name}.wav`);a.volume=volume;a.play().catch(()=>{})};
